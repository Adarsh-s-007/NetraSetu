...
g.followUp = struct('ROUTINE', 'Re-screen in 12 months', ...
    ...
    'REFER_DME', 'Ophthalmologist within 2 weeks (macular oedema risk)', ...
...
% environment overrides: keep several trained models / result sets apart
% (NETRA_DATA, read by netra.io.dataRoot, points at the datasets)
if ~isempty(getenv('NETRA_MODELS')), cfg.paths.models = getenv('NETRA_MODELS'); end
if ~isempty(getenv('NETRA_RESULTS')), cfg.paths.results = getenv('NETRA_RESULTS'); end
